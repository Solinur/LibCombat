...
function EventHandler:RegisterEvent(event, callback, ...) -- convinience function
	...
	if active then
		libint.totalevents = libint.totalevents + 1
	end
...
function EventHandler:UnregisterEvents()
	...
	if self.resetIds then
		ZO_ClearTable(libint.registeredSkills)
	end
end
...
function lf.UpdateResources(name, callbacktype, callback)
	if ActiveCallbackTypes[callbacktype] == nil then
		return false
	end
...
function libint.InitializeEvents()
	...
	EVENT_MANAGER:RegisterForEvent("LibCombatActive", EVENT_PLAYER_ACTIVATED, function()
		ld.isUIActivated = true
	end)
	EVENT_MANAGER:RegisterForEvent("LibCombatActive", EVENT_PLAYER_DEACTIVATED, function()
		ld.isUIActivated = false
	end)
