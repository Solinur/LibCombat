		...
		for i = 0, length - 1 do
			...
			if timeMs - data[1] < deathRecapTimePeriod then
				...
				local sourceUnit = sourceUnitId
						and sourceUnitId > 0
						and libint.currentFight
						and libint.currentFight.units
						and libint.currentFight.units[sourceUnitId]
					or "nil"
				--sourceUnitId == location ??
				data[3] = (sourceUnit and sourceUnit.name) or "Unknown"

				if data[10] and self.magickaMax and data[10] > self.magickaMax then
					self.magickaMax = data[10]
				end
				if data[11] and self.staminaMax and data[11] > self.staminaMax then
...
local function GetUnitCache(unitId)
	...
	if unitCache == nil then
		unitCache = UnitDeathCacheHandler:New(unitId)
	end
...
local function CheckForWipe() -- TODO use preassembled group unit tags
	...
	elseif libunits.inGroup == true then
		...
		for i = 1, GetGroupSize() do
			...
			if unit and not unit.isDead and GetUnitZoneIndex(unitTag) == playerZoneIndex then
				return
			end -- if there is a group member in the same zone but not dead then it's not a wipe
		end
	end
...
local function OnResurrect(
	...
	local timeMs = GetGameTimeMilliseconds()
	if targetUnitId == nil or targetUnitId == 0 or ld.inCombat == false then
		return
	end
	local unitdata = libint.currentFight.units[targetUnitId]
...
libint.Events.Deaths = libint.EventHandler:New({ LIBCOMBAT_LOG_EVENT_DEATH, LIBCOMBAT_EVENT_DEATHRECAP }, function(self)
...
libint.Events.DeathRecap = libint.EventHandler:New({ LIBCOMBAT_EVENT_DEATHRECAP }, function(self)
	local filters = {
