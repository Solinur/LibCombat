...
local function onSlotUsed(_, slot)
	...
	if libint.Events.Resources.active and slot > 2 and NonContiguousCount(powerTypes) > 0 then
		...
		if #lastabilities > libint.ABILITY_RESOURCE_CACHE_SIZE then
...
local function GetPlayerSprintState()
	for slot = 3, 8 do
		local anyAbilityActive = not (
			ActionSlotHasTargetFailure(slot, HOTBAR_CATEGORY_PRIMARY)
			and ActionSlotHasNonCostStateFailure(slot, HOTBAR_CATEGORY_PRIMARY)
			and ActionSlotHasTargetFailure(slot, HOTBAR_CATEGORY_BACKUP)
			and ActionSlotHasNonCostStateFailure(slot, HOTBAR_CATEGORY_PRIMARY)
		)

		if anyAbilityActive then
...
local function checkLastAbilities(timeMs, powerType, powerValueChange, powerValue)
	...
	for i = #lastabilities, 1, -1 do
		...
		logger:Debug(
			"Check: %s (%d), %d, %d",
			GetFormattedAbilityName(values[2]),
			values[2],
			powerValueChange / values[3],
			values[4]
		)
...
local function onBaseResourceChanged(powerType, powerValue, powerValueChange)
	...
	if powerType == COMBAT_MECHANIC_FLAGS_MAGICKA then
		...
		if
			...
			or (
				...
				and powerValue == ld.stats[LIBCOMBAT_STAT_MAXMAGICKA]
			)
		then
			abilityId = 0
	...
	lf.FireCallback(LIBCOMBAT_LOG_EVENT_RESOURCE, timeMs, abilityId, powerValueChange, powerType, powerValue)
end

local function onBaseResourceChangedDelayed(_, unitTag, _, powerType, newValue, _, _)
	if unitTag ~= "player" or (ld.inCombat == false) then
		return
	end
	if
		powerType ~= COMBAT_MECHANIC_FLAGS_HEALTH
	...
	if powerType == COMBAT_MECHANIC_FLAGS_HEALTH and ld.statusEffectBonus and ld.statusEffectBonus.wealdBonus > 0 then
		lf.UpdateSingleStat(LIBCOMBAT_STAT_STATUS_EFFECT_CHANCE)
	end
...
libint.Events.Resources = libint.EventHandler:New({ LIBCOMBAT_LOG_EVENT_RESOURCE }, function(self)
