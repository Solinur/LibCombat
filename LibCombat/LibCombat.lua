...
-- Basic values
lib.name = "LibCombat"
lib.data = { skillBars = {}, scribedSkills = {} }
...
local function Log(category, level, ...)
	...
	if level == LOG_LEVEL_DEBUG or level == LOG_LEVEL_VERBOSE then
		return
	end
	local logger = category and logger[category] or logger
	if category == "debug" and lib.debug ~= true then
		return
	end
...
local reset = false
local data = lib.data
lib.debug = false or (SoliTest ~= nil)
local timeout = 800
...
local SpecialBuffs =
	{ -- buffs that the API doesn't show via EVENT_EFFECT_CHANGED and need to be specially tracked via EVENT_COMBAT_EVENT
...
local CustomAbilityName = {
	...
	[113382] = zo_strformat(
		SI_LIBCOMBAT_CUSTOM_ABILITY_FORMAT,
		GetAbilityName(113382),
		GetString(SI_LIBCOMBAT_LOG_DEBUFF)
	), -- To make sure that tracking works correctly since both buff and debuff are named the same.
	...
	[61919] = zo_strformat(
		...
		GetString(SI_ABILITY_TOOLTIP_TOGGLE_DURATION)
	), -- Merciless Resolve Toggle
	[61927] = zo_strformat(
		SI_LIBCOMBAT_CUSTOM_ABILITY_FORMAT,
		GetAbilityName(61927),
		GetString(SI_ABILITY_TOOLTIP_TOGGLE_DURATION)
	), -- Relentless Focus Toggle

	[122729] = zo_strformat(
		SI_LIBCOMBAT_CUSTOM_ABILITY_FORMAT,
		GetAbilityName(122729),
		GetString(SI_LIBCOMBAT_LOG_BUFF)
	), --  Name for separate stats buff of Seething Fury
}
...
function UnitHandler:Initialize(name, unitId, unitType)
	...
	if unitType == nil or unitType == COMBAT_UNIT_TYPE_TARGET_DUMMY then
	...
	if
		...
		or unitType == COMBAT_UNIT_TYPE_GROUP
	...
	if self.unitType == COMBAT_UNIT_TYPE_GROUP then
...
function UnitHandler:UpdateGroupData()
	...
	if unitTag then
		self.isDead = IsUnitDeadOrReincarnating(unitTag)
	end

	groupdata.nameToId[self.name] = self.unitId

	if unitTag then
		groupdata.tagToId[unitTag] = self.unitId
	end
end
...
function UnitHandler:UpdateZenData(
	callbackKeys,
	eventid,
	timeMs,
	unitId,
	...
	if abilityId == abilityIdZen then
		...
		lib.cm:FireCallbacks(
			...
			sourceType,
			effectSlot
		) -- stack count is 1 to 6, with 1 meaning 0% bonus, and 6 meaning 5% bonus from Z'en
		Log(
			"debug",
			LOG_LEVEL_VERBOSE,
			table.concat(
				{ eventid, timeMs, unitId, abilityIdZen, changeType, effectType, stacks, sourceType, effectSlot },
				", "
			)
		)
	...
	elseif abilityType == ABILITY_TYPE_DAMAGE then
		...
		if self.zenEffectSlot then
			...
			lib.cm:FireCallbacks(
				CallbackKeys[LIBCOMBAT_EVENT_EFFECTS_OUT],
...
function UnitHandler:UpdateForceOfNatureData(_, _, timeMs, unitId, abilityId, changeType, _, _, _, _)
	if
		StatusEffectIds[abilityId] == nil
		or currentfight.CP[1]["slotted"] == nil
		or currentfight.CP[1]["slotted"][276] ~= true
	then
		return
	end
	...
	elseif changeType == EFFECT_RESULT_FADED and self.forceOfNature[abilityId] == true then
		...
		if self.forceOfNatureStacks == 0 then
			forceOfNatureChangeType = EFFECT_RESULT_FADED
		end
		if self.forceOfNatureStacks - 1 < 0 then
			Log(
				"debug",
				LOG_LEVEL_WARNING,
...
local function onDuelStart() -- for duels, where resetting combat sometimes fails
	...
	if currentfight.prepared == false then
		currentfight:PrepareFight()
	end
end
...
local function GetPlayerBuffs(timems)
	...
	if Events.Effects.active == false then
		return
	end
	...
	for i = 1, GetNumBuffs("player") do
		...
		local _, _, endTime, effectSlot, stackCount, _, _, effectType, abilityType, _, abilityId, _, castByPlayer =
		...
		if not BadAbility[abilityId] then
			lib.cm:FireCallbacks(
				CallbackKeys[LIBCOMBAT_EVENT_EFFECTS_IN],
...
local function GetOtherBuffs(timems)
	...
	for unitId, unitData in pairs(EffectBuffer) do
		for abilityId, abilityData in pairs(unitData) do
			...
			if sourceType ~= COMBAT_UNIT_TYPE_PLAYER or abilityId ~= abilityIdZen then
				lib.cm:FireCallbacks(CallbackKeys[logdata[1]], unpack(logdata))
			end

			if
				sourceType == COMBAT_UNIT_TYPE_PLAYER
				and (abilityId == abilityIdZen or abilityType == ABILITY_TYPE_DAMAGE)
			then
				local unit = currentfight.units[unitId]
				if unit then
					unit:UpdateZenData(CallbackKeys[logdata[1]], unpack(logdata), abilityType)
				end
			end
			...
			if sourceType == COMBAT_UNIT_TYPE_PLAYER and StatusEffectIds[abilityId] then
				local unit = currentfight.units[unitId]
				if unit then
					unit:UpdateForceOfNatureData(CallbackKeys[logdata[1]], unpack(logdata))
				end
			end
		end
	end
	...
	for _, bar in pairs(data.skillBars) do
		for _, abilityId in pairs(bar) do
			if registeredIds[abilityId] == nil then
				...
				if abilityAdditions[abilityId] then
					table.insert(SlotSkills, GetSkillRegistrationData(abilityAdditions[abilityId]))
				end
			end
		end
	end
...
local function onBossesChanged(_) -- Detect Bosses
	...
	for i = 1, 12 do
		...
		if DoesUnitExist(unitTag) then
			...
			if currentfight.bossname == nil and name ~= nil and name ~= "" then
...
function FightHandler:PrepareFight()
	...
	if not success then
		zo_callLater(function()
			em:RegisterForUpdate("LibCombat_update", 500, onUpdate)
		end, 1500)
	end
end
...
local function GetAdvancedStats()
	...
	for statType, _ in pairs(advancedStatData) do
		...
		if flatValue then
			advancedStatData[statType][1] = flatValue
		end
		if percentValue then
			advancedStatData[statType][2] = percentValue
		end
	end
...
function FightHandler:UpdateSingleStat(statId, timems)
	if Events.Stats.active ~= true then
		return
	end
	...
	if lastcalldelta < 100 then
		em:RegisterForUpdate("LibCombat_Stats_Single", (100 - lastcalldelta), function()
			self:UpdateSingleStat(statId, nil)
		end)
		return
	end
	...
	if oldValue == nil or delta ~= 0 then
		...
		assert(statId ~= nil)
		lib.cm:FireCallbacks(
			CallbackKeys[LIBCOMBAT_EVENT_PLAYERSTATS],
...
function FightHandler:GetNewStats(timems)
	...
	if lastcalldelta < 100 then
		em:RegisterForUpdate("LibCombat_Stats", (100 - lastcalldelta), function()
			self:GetNewStats()
		end)
		return
	end
	...
	if Events.AdvancedStats.active ~= true then
		return
	end
	local advancedStats = data.advancedStats

	for statId, values in pairs(GetAdvancedStats()) do
		if advancedStats[statId] == nil then
			advancedStats[statId] = {}
		end
		...
		if newValue1 then
			...
			if oldValue == nil or delta ~= 0 then
				lib.cm:FireCallbacks(
					CallbackKeys[LIBCOMBAT_EVENT_PLAYERSTATS_ADVANCED],
...
function FightHandler:UpdateStats()
	...
	if (self.dpsend == nil and self.hpsend == nil) or (self.dpsstart == nil and self.hpsstart == nil) then
		return
	end
...
function FightHandler:UpdateGrpStats() -- called by onUpdate
	if not (self.group and Events.CombatGrp.active) then
		return
	end
...
function FightHandler:onUpdate()
	...
	if
		reset == true
		or (data.inCombat == false and self.combatend > 0 and (GetGameTimeMilliseconds() > (self.combatend + timeout)))
	then
		reset = false
...
function UnitCacheHandler:ProcessDeath()
	...
	if unit then
		ZO_ShallowTableCopy(unit:GetUnitInfo(), self)
	end
	...
	if #cache > 0 then
		...
		for i = 0, length - 1 do
			...
			if timems - data[1] < deathRecapTimePeriod then
				...
				local sourceUnit = sourceUnitId
						and sourceUnitId > 0
						and currentfight
						and currentfight.units
						and currentfight.units[sourceUnitId]
					or "nil"

				data[3] = (sourceUnit and sourceUnit.name) or "Unknown"

				if data[10] and data[10] > self.magickaMax then
					self.magickaMax = data[10]
				end
				if data[11] and data[11] > self.staminaMax then
					self.staminaMax = data[11]
				end
			else
				deleted = deleted + 1
			end
...
local function GetUnitCache(unitId)
	if unitId == nil or unitId < 1 then
		return
	end
	local unitCache = CombatEventCache[unitId]
	if unitCache == nil then
		unitCache = UnitCacheHandler:New(unitId)
	end
...
function onCombatState(event, inCombat) -- Detect Combat Stage, local is defined above - Don't Change !!!
	if inCombat ~= data.inCombat then -- Check if player state changed
		...
		if inCombat then
			...
			lib.cm:FireCallbacks(
				CallbackKeys[LIBCOMBAT_EVENT_MESSAGES],
			...
			if currentfight.prepared and currentfight.combatend < 0 then
				currentfight:FinishFight()
			end

			if currentfight.charData == nil then
				return
			end
			lib.cm:FireCallbacks(
				CallbackKeys[LIBCOMBAT_EVENT_MESSAGES],
...
local function onTrialDummy(_, _, _, _, _, _, _, _, _, _, _, _, _, _, sourceUnitId, _, _, _)
	if not currentfight.prepared then
		return
	end
...
local function BuffEventHandler(
	isspecial,
	groupeffect,
	_,
	changeType,
	...
	if
		(
			...
			and not (changeType == EFFECT_RESULT_UPDATED and stackCount > 1)
		)
		or unitName == "Offline"
		or unitId == nil
	then
	...
	if BadAbility[abilityId] == true then
		return
	end
	...
	if
		unitTag
		and zo_strsub(unitTag, 1, 11) ~= "reticleover"
		and (
			AreUnitsEqual(unitTag, "reticleover")
			or AreUnitsEqual(unitTag, "reticleoverplayer")
			or AreUnitsEqual(unitTag, "reticleovertarget")
		)
	then
		return
	end
	...
	local eventid = groupeffect == GROUP_EFFECT_IN and LIBCOMBAT_EVENT_GROUPEFFECTS_IN
		or groupeffect == GROUP_EFFECT_OUT and LIBCOMBAT_EVENT_GROUPEFFECTS_OUT
		or unitTag and zo_strsub(unitTag, 1, 6) == "player" and LIBCOMBAT_EVENT_EFFECTS_IN
		or LIBCOMBAT_EVENT_EFFECTS_OUT
	...
	if
		inCombat ~= true
		and unitTag ~= "player"
		and (changeType == EFFECT_RESULT_GAINED or changeType == EFFECT_RESULT_UPDATED)
	then
		AddtoEffectBuffer(
			endTime,
			abilityType,
	...
	elseif inCombat == true then
		...
		if unitTag == "player" or unitId == data.playerid then
			currentfight:GetNewStats(timems)
		end
		if sourceType ~= COMBAT_UNIT_TYPE_PLAYER or abilityId ~= abilityIdZen then
			lib.cm:FireCallbacks(
				CallbackKeys[eventid],
				eventid,
				timems,
				unitId,
				abilityId,
		...
		if unit then
			...
			if
				StatusEffectIds[abilityId]
				and (
					sourceType == COMBAT_UNIT_TYPE_PLAYER
					or (unitName == "" and unit.forceOfNature[abilityId] and SpecialDebuffs[abilityId])
				)
			then
				unit:UpdateForceOfNatureData(
					CallbackKeys[eventid],
					eventid,
					timems,
					unitId,
...
local function onSourceBuggedEffectChanged(
	eventCode,
	changeType,
	effectSlot,
	effectName,
...
local function SpecialBuffEventHandler(
	isdebuff,
	_,
	result,
	_,
	...
	if BadAbility[abilityId] == true or (result == ACTION_RESULT_EFFECT_GAINED and hitValue < 2) then
		return
	end
	...
	local unitTag = currentfight.units and currentfight.units[targetUnitId] and currentfight.units[targetUnitId].unitTag
		or nil
...
local function onShadowMundus(_, changeType, effectSlot)
	...
	if currentfight.prepared == true then
		currentfight:GetNewStats()
	end
end
...
local function GetPlayerSprintState()
	...
	for slot = 3, 8 do
		...
		if button and button.usable then
		...
		if powerType == values[4] then
			...
			elseif values[2] == 58431 and GetAllyUnitBlockState("player") == BLOCK_STATE_ACTIVE then -- check if Constitution coincides with Block
				...
				if goodratio then
					...
					lib.cm:FireCallbacks(
						CallbackKeys[LIBCOMBAT_EVENT_RESOURCES],
...
local function checkForCombatActions(powerValueChange)
	...
	if
		...
		or GetUnitStealthState("player") == STEALTH_STATE_HIDDEN and powerValueChange < 0 and powerValueChange > -20
	then
...
local function onBaseResourceChanged(powerType, powerValue, powerValueChange)
	...
	if powerType == COMBAT_MECHANIC_FLAGS_MAGICKA then
		...
		if
			abilityId == -1 and powerValueChange == regenerationTick
			or (
				powerValueChange > 0
				and powerValueChange <= regenerationTick
				and powerValue == data.stats[LIBCOMBAT_STAT_MAXMAGICKA]
			)
		then
			abilityId = 0
	...
	elseif powerType == COMBAT_MECHANIC_FLAGS_STAMINA then
		...
		if adjustedPowerValueChange then
			powerValueChange = adjustedPowerValueChange
		end
		...
		elseif abilityId == -1 then -- Check for combat actions
			...
			if abilityId == -1 then -- Check for combination of skill and regeneration tick
				abilityId, adjustedPowerValueChange =
					checkLastAbilities(timems, powerType, powerValueChange + regenerationTick, powerValue)
	...
	elseif powerType == COMBAT_MECHANIC_FLAGS_HEALTH then
		...
		if powerValueChange == GetStat(STAT_HEALTH_REGEN_COMBAT) and data.playerid then
			abilityId = 0
			lib.cm:FireCallbacks(
				CallbackKeys[LIBCOMBAT_EVENT_HEAL_SELF],
...
local function onBaseResourceChangedDelayed(_, unitTag, _, powerType, newValue, _, _)
	if unitTag ~= "player" or (data.inCombat == false) then
		return
	end
	if
		...
		and powerType ~= COMBAT_MECHANIC_FLAGS_ULTIMATE
	then
		return
	end
	...
	if
		powerType == COMBAT_MECHANIC_FLAGS_HEALTH
		and data.statusEffectBonus
		and data.statusEffectBonus.wealdBonus > 0
	then
		currentfight:UpdateSingleStat(LIBCOMBAT_STAT_STATUS_EFFECT_CHANCE)
	end
...
local function onResourceChanged(
	...
	if data.playerid == nil and targetName == data.rawPlayername then
		data.playerid = targetUnitId
	end
	if
		(powerType ~= COMBAT_MECHANIC_FLAGS_MAGICKA and powerType ~= COMBAT_MECHANIC_FLAGS_STAMINA)
		or data.inCombat == false
		or powerValueChange < 1
	then
		return
	end
	...
	if #lastabilities > ABILITY_RESOURCE_CACHE_SIZE then
...
local function CheckForWipe()
	if not IsUnitDeadOrReincarnating("player") then
		return
	end -- maybe it's enough if player is dead on combat end? (Unless it bugs, like Sunspire ... ¯\_(ツ)_/¯)
	...
	elseif data.inGroup == true then
		...
		for i = 1, GetGroupSize() do
			...
			if unit and not unit.isDead and GetUnitZoneIndex(tag) == loc then
				return
			end -- if there is a group member in the zame zone but not dead then it's not a wipe
		end
	end
...
local function OnDeathStateChanged(_, unitTag, isDead) -- death (for group display, also works for different zones)
	...
	if isDead then
		...
		if lasttime and lasttime - timems < 1000 then
			return
		end
		...
		lib.cm:FireCallbacks(
			CallbackKeys[LIBCOMBAT_EVENT_DEATH],
...
local function OnDeath(
	...
	if
		unitdata == nil
		or (unitdata.unitType ~= COMBAT_UNIT_TYPE_PLAYER and unitdata.unitType ~= COMBAT_UNIT_TYPE_GROUP)
	then
		return
	end
...
local function OnResurrect(
	...
	if targetUnitId == nil or targetUnitId == 0 or data.inCombat == false then
		return
	end

	local unitdata = currentfight.units[targetUnitId]

	if unitdata == nil or unitdata.type ~= COMBAT_UNIT_TYPE_GROUP then
		return
	end
...
local function CheckUnit(unitName, unitId, unitType, timems)
	...
	if currentunits[unitId] == nil then
		currentunits[unitId] = UnitHandler:New(unitName, unitId, unitType)
	end
	local unit = currentunits[unitId]

	if unit.name == "Offline" or unit.name == "" then
...
local function CombatEventHandler(
	isheal,
	_,
	result,
	_,
	...
	if
		not (sourceUnitId > 0 and targetUnitId > 0)
		or (data.inCombat == false and (result == ACTION_RESULT_DOT_TICK_CRITICAL or result == ACTION_RESULT_DOT_TICK or isheal))
		or targetType == 2
	then
		return
	end -- only record if both unitids are valid or player is in combat or a non dot damage action happens or the target is not a pet

	local timems = GetGameTimeMilliseconds()

	local shieldHitValue = CheckForShield(timems, sourceUnitId, targetUnitId) or 0

	if (hitValue + (overflow or 0) + shieldHitValue) <= 0 then
		return
	end
...
local function onCombatEventShield(
	eventCode,
	result,
	isError,
	abilityName,
...
local function onCombatEventDmgIn(...)
	...
	if
		(sourceType == COMBAT_UNIT_TYPE_PLAYER or sourceType == COMBAT_UNIT_TYPE_PLAYER_PET)
		and (targetType == COMBAT_UNIT_TYPE_PLAYER or targetType == COMBAT_UNIT_TYPE_PLAYER_PET)
	then
		return
	end
...
local function onCombatEventHeal(...)
	...
	if
		(hitValue + (overflow or 0)) < 2
		or (data.inCombat == false and (GetGameTimeMilliseconds() - currentfight.combatend >= 50))
	then
		return
	end -- only record in combat
...
local function onCombatEventDmgGrp(
	_,
	_,
	_,
	_,
	...
	if hitValue < 2 or targetUnitId == nil or targetType == 2 then
		return
	end
...
local function onCombatEventHealGrp(_, _, _, _, _, _, _, _, _, targetType, hitValue, _, _, _, _, targetUnitId, _) -- called by Event
	if
		...
		or targetUnitId == nil
		or hitValue < 2
		or (data.inCombat == false and (GetGameTimeMilliseconds() - (currentfight.combatend or 0) >= 50))
	then
		return
	end
...
local function GroupCombatEventHandler(
	isheal,
	result,
	_,
	abilityName,
	...
	if
		(hitValue + (overflow or 0)) < 0
		or not (targetUnitId > 0)
		or (
			data.inCombat == false
			and (result == ACTION_RESULT_DOT_TICK_CRITICAL or result == ACTION_RESULT_DOT_TICK or isheal)
		)
	then
		return
	end -- only record if both unitids are valid or player is in combat or a non dot damage action happens
...
local function onCombatEventGrpDmgIn(event, ...)
	...
	if not targetType or (targetType ~= COMBAT_UNIT_TYPE_GROUP and targetType ~= COMBAT_UNIT_TYPE_PLAYER) then
		return
	end
...
local function onBaseResourceChangedGroup(
	event,
	unitTag,
	powerIndex,
	powerType,
	...
	if unitId then
		GetUnitCache(unitId):UpdateResource(powerType, powerValue, powerMax)
	end
end
...
local function onAbilityUsed(
	...
	if
		Events.Skills.active ~= true
		or data.inCombat == false
		or not (
			validSkillStartResults[result]
			or (validNonProjectileSkillStartResults[result] and not isProjectile[abilityId])
		)
	then
		return
	end
	...
	local reducedslot = IdToReducedSlot[abilityId]
		or (abilityAdditionsReverse[abilityId] and IdToReducedSlot[abilityAdditionsReverse[abilityId]])
		or nil
	...
	if castTime > 0 then
		...
		lib.cm:FireCallbacks(
			CallbackKeys[LIBCOMBAT_EVENT_SKILL_TIMINGS],
...
local function onQueueEvent(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, abilityId)
	...
	if reducedslot == nil then
		Log(
			...
			(timems - currentfight.combatstart) / 1000,
...
local function onProjectileEvent(
	...
	if hitValue == nil or hitValue <= 1 or targetType == COMBAT_UNIT_TYPE_PLAYER or isProjectile[abilityId] == true then
		return
	end
	...
	Log(
		...
		LOG_LEVEL_VERBOSE,
		"[%.3f s] projectile: %s (%d)",
		(GetGameTimeMilliseconds() - currentfight.combatstart) / 1000,
		GetAbilityName(abilityId),
		abilityId
	)
...
local function onSlotUsed(_, slot)
	if data.inCombat == false or slot > 8 then
		return
	end
...
local function onBossHealthChanged(eventid, unitTag, _, powerType, powerValue, powerMax, powerEffectiveMax)
	...
	if BossHealthValue == lastBossHealthValue then
		return
	end
	...
	lib.cm:FireCallbacks(
		CallbackKeys[LIBCOMBAT_EVENT_BOSSHP],
...
local function onFrameUpdate()
	...
	if now == currentsecond then
		...
		lib.cm:FireCallbacks(
			CallbackKeys[LIBCOMBAT_EVENT_PERFORMANCE],
...
local function onQuickSlotUsed(_, itemSoundCategory)
	...
	if itemSoundCategory ~= GetSlotItemSound(data.currentQuickslotIndex, HOTBAR_CATEGORY_QUICKSLOT_WHEEL) then
		return
	end
...
function EventHandler:RegisterEvent(event, callback, ...) -- convinience function
	...
	if active then
		lib.totalevents = lib.totalevents + 1
	end

	if data.isUIActivated and event == EVENT_PLAYER_ACTIVATED then
...
function EventHandler:UpdateEvents()
	...
	for k, v in pairs(self.callbacktypes) do
		if NonContiguousCount(ActiveCallbackTypes[v]) > 0 then
			condition = true
			break
		end
	end
...
if GetAPIVersion() > 100027 then
	EVENT_ACTION_SLOT_ABILITY_SLOTTED = EVENT_HOTBAR_SLOT_CHANGE_REQUESTED
end

Events.General = EventHandler:New(GetAllCallbackTypes(), function(self)
...
Events.CombatGrp = EventHandler:New({ LIBCOMBAT_EVENT_FIGHTRECAP, LIBCOMBAT_EVENT_GROUPRECAP }, function(self)
	local filters = {
		[onCombatEventDmgGrp] = {
			ACTION_RESULT_DAMAGE,
			ACTION_RESULT_DOT_TICK,
...
Events.GroupEffectsOut = EventHandler:New({ LIBCOMBAT_EVENT_GROUPEFFECTS_OUT }, function(self)
	self:RegisterEvent(
		EVENT_EFFECT_CHANGED,
		onGroupEffectOut,
		REGISTER_FILTER_SOURCE_COMBAT_UNIT_TYPE,
...
Events.Stats = EventHandler:New({ LIBCOMBAT_EVENT_PLAYERSTATS }, function(self)
	self:RegisterEvent(
		EVENT_EFFECT_CHANGED,
		onShadowMundus,
		REGISTER_FILTER_UNIT_TAG,
...
Events.AdvancedStats = EventHandler:New({ LIBCOMBAT_EVENT_PLAYERSTATS_ADVANCED }, function(self)
	self.active = true
end)

Events.Resources = EventHandler:New({ LIBCOMBAT_EVENT_RESOURCES }, function(self)
...
Events.Deaths = EventHandler:New({ LIBCOMBAT_EVENT_DEATH, LIBCOMBAT_EVENT_DEATHRECAP }, function(self)
...
Events.DeathRecap = EventHandler:New({ LIBCOMBAT_EVENT_DEATHRECAP }, function(self)
	local filters = {
		ACTION_RESULT_DAMAGE,
		ACTION_RESULT_DOT_TICK,
		ACTION_RESULT_BLOCKED_DAMAGE,
...
Events.Slots = EventHandler:New({ LIBCOMBAT_EVENT_RESOURCES, LIBCOMBAT_EVENT_SKILL_TIMINGS }, function(self)
	self:RegisterEvent(EVENT_ACTION_SLOT_ABILITY_USED, onSlotUsed)

	self.active = true
end)

Events.Skills = EventHandler:New({ LIBCOMBAT_EVENT_SKILL_TIMINGS }, function(self)
	self:RegisterEvent(
		EVENT_COMBAT_EVENT,
		GetCurrentSkillBarsDelayed,
		REGISTER_FILTER_COMBAT_RESULT,
...
Events.BossHP = EventHandler:New({ LIBCOMBAT_EVENT_BOSSHP }, function(self)
	self:RegisterEvent(
		EVENT_POWER_UPDATE,
		onBossHealthChanged,
		REGISTER_FILTER_UNIT_TAG,
...
Events.Performance = EventHandler:New({ LIBCOMBAT_EVENT_PERFORMANCE }, function(self)
...
Events.QuickSlot = EventHandler:New({ LIBCOMBAT_EVENT_QUICKSLOT }, function(self)
...
local function GetAbilityString(abilityId, damageType, fontsize, showIds, stacks)
	...
	local format = abilityId == abilityIdZen and "<<5[/$dx /$dx ]>><<1>> <<2>><<3>><<4[/ ($d)/ ($d)]>>|r"
...
function lib:GetCombatLogString(fight, logline, fontsize, showIds)
	...
	if logtype == LIBCOMBAT_EVENT_DAMAGE_OUT then
		...
		local crit = (result == ACTION_RESULT_CRITICAL_DAMAGE or result == ACTION_RESULT_DOT_TICK_CRITICAL)
				and ZO_CachedStrFormat("|cFFCC99<<1>>|r", GetString(SI_LIBCOMBAT_LOG_CRITICAL))
			or ""

		local targetname = units[targetUnitId].name
		local targetFormat = (result == ACTION_RESULT_BLOCKED_DAMAGE and SI_LIBCOMBAT_LOG_FORMAT_TARGET_BLOCK)
			or SI_LIBCOMBAT_LOG_FORMAT_TARGET_NORMAL
		...
		local hitValueString = overflow > 0
				and ZO_CachedStrFormat(GetString(SI_LIBCOMBAT_LOG_FORMAT_ABSORBED), hitValue, overflow)
			or hitValue
	...
	elseif logtype == LIBCOMBAT_EVENT_DAMAGE_IN then
		...
		local targetFormat = (result == ACTION_RESULT_BLOCKED_DAMAGE and SI_LIBCOMBAT_LOG_FORMAT_TARGETSELF_BLOCK)
			or SI_LIBCOMBAT_LOG_FORMAT_TARGETSELF_NORMAL
	...
	elseif logtype == LIBCOMBAT_EVENT_DAMAGE_SELF then
		...
		local crit = (result == ACTION_RESULT_CRITICAL_HEAL or result == ACTION_RESULT_HOT_TICK_CRITICAL)
				and ZO_CachedStrFormat("|cFFCC99<<1>>|r", GetString(SI_LIBCOMBAT_LOG_CRITICAL))
			or ""

		local targetFormat = (result == ACTION_RESULT_BLOCKED_DAMAGE and SI_LIBCOMBAT_LOG_FORMAT_TARGETSELF_BLOCK)
			or SI_LIBCOMBAT_LOG_FORMAT_TARGETSELF_SELF
		local targetString = GetString(targetFormat)
	...
	elseif logtype == LIBCOMBAT_EVENT_HEAL_SELF then
		...
		color = { 0.8, 1.0, 0.6 }
		text = result == ACTION_RESULT_DAMAGE_SHIELDED
				and ZO_CachedStrFormat(GetString(SI_LIBCOMBAT_LOG_FORMAT_HEALABSORB), timeString, ability, hitValue)
			or ZO_CachedStrFormat(logFormat, timeString, crit, ability, hitValue)
	elseif
	...
	then
		...
		local changeTypeString = (changeType == EFFECT_RESULT_GAINED or changeType == EFFECT_RESULT_UPDATED)
				and GetString(SI_LIBCOMBAT_LOG_GAINED)
			or changeType == EFFECT_RESULT_FADED and GetString(SI_LIBCOMBAT_LOG_LOST)

		local source = UnitTypeString[sourceType] == nil and ""
			or ZO_CachedStrFormat(" from <<1>>", UnitTypeString[sourceType])
	...
	elseif logtype == LIBCOMBAT_EVENT_RESOURCES then
		...
		if powerValueChange ~= nil then
			...
			local resource = (powerType == COMBAT_MECHANIC_FLAGS_MAGICKA and GetString(SI_ATTRIBUTES2))
				or (powerType == COMBAT_MECHANIC_FLAGS_STAMINA and GetString(SI_ATTRIBUTES3))
				or (powerType == COMBAT_MECHANIC_FLAGS_ULTIMATE and GetString(SI_LIBCOMBAT_LOG_ULTIMATE))

			local ability = abilityId
					and ZO_CachedStrFormat("(<<1>>)", GetAbilityString(abilityId, "resource", fontsize, showIds))
				or ""

			color = (powerType == COMBAT_MECHANIC_FLAGS_MAGICKA and { 0.7, 0.7, 1 })
				or (powerType == COMBAT_MECHANIC_FLAGS_STAMINA and { 0.7, 1, 0.7 })
				or (powerType == COMBAT_MECHANIC_FLAGS_ULTIMATE and { 1, 1, 0.7 })
				or color
	...
	elseif logtype == LIBCOMBAT_EVENT_MESSAGES then
		...
		if message == LIBCOMBAT_MESSAGE_WEAPONSWAP then
			...
			messagetext = ZO_CachedStrFormat(
				formatstring,
				GetString(SI_LIBCOMBAT_LOG_MESSAGE3),
				GetString(SI_LIBCOMBAT_LOG_MESSAGE_BAR),
				bar
			)
	...
	elseif logtype == LIBCOMBAT_EVENT_SKILL_TIMINGS then
		...
		local skillDelayString = skillDelay
				and ZO_CachedStrFormat(GetString(SI_LIBCOMBAT_LOG_FORMATSTRING_SKILLDELAY), skillDelay)
			or ""
	...
	elseif logtype == LIBCOMBAT_EVENT_PERFORMANCE then
		...
		local pingcolor = ping <= 50 and "99ff99"
			or ping <= 80 and "ccff99"
			or ping <= 120 and "ffff99"
			or ping <= 160 and "ffcc99"
			or "ff9999"
		local pingString = ZO_CachedStrFormat("|c<<1>><<2>>|r", pingcolor, ping)

		local fpsColor = fps >= 100 and "99ff99"
			or fps >= 60 and "ccff99"
			or fps >= 40 and "ffff99"
			or fps >= 25 and "ffcc99"
			or "ff9999"
		local fpsString = ZO_CachedStrFormat("|c<<1>><<2>>|r", fpsColor, fps)

		local minString = min < (0.5 * fps) and min < 25 and ZO_CachedStrFormat("|cff9999<<1>>|r", min)
			or min < (0.7 * fps) and min < 40 and ZO_CachedStrFormat("|cffddbb<<1>>|r", min)
			or min
...
local function Initialize()
	...
	if data.LoadCustomizations then
		data.LoadCustomizations()
	end

	em:RegisterForEvent("LibCombatActive", EVENT_PLAYER_ACTIVATED, function()
		data.isUIActivated = true
	end)
	em:RegisterForEvent("LibCombatActive", EVENT_PLAYER_DEACTIVATED, function()
